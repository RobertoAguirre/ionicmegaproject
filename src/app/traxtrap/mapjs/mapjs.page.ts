...
export class MapjsPage implements OnInit {
  ...
  createMap() {
    ...
    this.myMap = new google.maps.Map(this.mapRef.nativeElement, this.mapOptions);
  ...
  turnLocationsIntoMarkers(_map,_markers) {
    ...
    Object.values(this.locations.locations).forEach(function (value: any) {
      ...
      let position = new google.maps.LatLng(value.lastrecord.latitude, value.lastrecord.longitude)
  ...
  updateMarkersPositions(_map){
    ...
    for(let marker of this.markers){
      ...
      let latlng = new google.maps.LatLng(currentMarker.lastrecord.latitude, currentMarker.lastrecord.longitude);
